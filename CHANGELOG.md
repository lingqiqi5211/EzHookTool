...
- 102 新增 `deoptimizeOrThrow`：框架调用抛出的异常原样传出，不再被压成布尔值。
- 102 首次就绪初始化的状态和失败原因可查询。
- 资源替换补齐 array、mipmap、plurals、字符串格式化和常用 TypedArray 值读取。
...
- 102 的 `deoptimize` 不再将框架返回的 `false` 误报为成功。
- `onTargetReady` 的重入注册按队尾执行，回调执行后释放，失败不自动重试。
- 实例附加字段按对象身份匹配，不再让相等对象共享字段或受可变 hashCode 影响。
- 资源 getter 统一按完整签名安装，待提交与已安装分开记录；TypedArray 不再依赖私有数据布局。
- 修复整数、颜色和尺寸的取值转换，以及模块资源的参数转发；数组登记和返回都使用副本。
