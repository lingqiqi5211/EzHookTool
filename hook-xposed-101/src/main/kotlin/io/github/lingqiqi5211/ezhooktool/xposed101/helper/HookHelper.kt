...
import io.github.lingqiqi5211.ezhooktool.xposed101.dsl.HookFactory
import java.lang.reflect.Constructor
import java.lang.reflect.Method
import java.util.function.Consumer

/**
 * 基于 [Method] 安装 intercept hook。
 *
 * 这是 libxposed 101 最接近原生 `Hooker` 的入口，适合需要自行决定是否 `proceed()` 的场景。
 *
 ...
 *
 * 需要组合 `before`、`after`、`replace`、`intercept` 时，优先用这个入口。
 *
 * @param priority hook 优先级，数值越大越先执行
 * @param exceptionMode hook 过程中异常的处理策略
...
/**
 * 基于 [Constructor] 安装 intercept hook。
 *
 * @param priority hook 优先级，数值越大越先执行
 * @param exceptionMode hook 过程中异常的处理策略
 * @param callback around 回调，可自行决定是否继续原始调用
