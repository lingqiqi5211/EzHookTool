    ...
    constructor(
        ...
        fun finishHotReload(): HookReloadBatchResult =
            synchronized(lock) {
                ...
                val snapshot = oldHooks ?: throw IllegalStateException(
                ...
                val currentDirectHooks = LinkedHashSet(explicitHooks.keys)
                val logicalHookCount = groups.values.sumOf { it.logicalHooks.size }
                ...
                oldHooks = null
                if (failures.isNotEmpty()) state = State.FAILED
                if (failures.isNotEmpty()) {
                    throw IllegalStateException(
                        "HookReloadBatch could not remove ${failures.size} obsolete old hook(s); restart the target process.",
                    ).also { error -> failures.forEach(error::addSuppressed) }
                }
                HookReloadBatchResult(
        ...
        internal val resourceHookState: ResourceHookState
            get() =
                synchronized(lock) {
                    when (state) {
                        State.NEW, State.INSTALLING -> ResourceHookState.PENDING
                        State.READY -> ResourceHookState.INSTALLED
                        State.FAILED -> ResourceHookState.FAILED
                    }
                }
        ...
        private fun removeExplicitHook(
            ...
            synchronized(lock) {
                ...
                when (state) {
                    ...
                    State.READY -> {
                        mutatePhysicalHook { hook.physicalHandle?.unhook() }
                        explicitHooks.remove(identity)
                    }

                    State.NEW, State.FAILED -> {
                        throw IllegalStateException("HookReloadBatch is no longer active.")
                    }
                }
            }
        }
        ...
        private fun replaceExplicitHook(
            ...
            synchronized(lock) {
                ...
                when (state) {
                    ...
                    State.READY -> {
                        val previous = hook.physicalHandle ?: throw IllegalStateException("Hook handle is no longer valid.")
                        hook.physicalHandle =
                            mutatePhysicalHook {
                                XposedApiCompat.Api102.replaceHook(previous, hooker)
                            }
                        hook.hooker = hooker
                        hook.token = replacementToken
                    }
        ...
        private fun commit() {
            synchronized(lock) {
                ...
                try {
                    ...
                    if (cleanupFailures.isNotEmpty()) EzXposed.markHotReloadIrreversible()
                ...
                if (pendingHooks.isNotEmpty()) EzXposed.markHotReloadIrreversible()
                state = State.READY
            }
        }

        private fun pendingPhysicalHooks(): List<PendingPhysicalHook> =
            buildList {
        ...
        private fun ensureStableTopologyBeforeCommit() {
            ...
            val newIdentities =
                LinkedHashSet<HookIdentity>().apply {
                    groups.values
                        .filter { it.logicalHooks.isNotEmpty() }
        ...
        // 安装期的物理发布统一由 commit 执行，避免后续初始化回调失败时已切换半代 Hook。

        private fun removeLogicalHook(
            key: GroupKey,
            token: Long,
        ) {
            synchronized(lock) {
                ...
                if (index < 0) return
                val candidate = group.logicalHooks.toMutableList().apply { removeAt(index) }
                publishMutation(group, candidate)
            }
        }
        ...
        private fun replaceLogicalHook(
            ...
            synchronized(lock) {
                ...
                val replacementToken = nextToken++
                val candidate =
                    group.logicalHooks.toMutableList().apply {
                        this[index] = LogicalHook(replacementToken, hooker)
                    }
                publishMutation(group, candidate)
        ...
        private fun publishMutation(
            group: HookGroup,
            candidate: List<LogicalHook>,
        ) {
            when (state) {
                State.INSTALLING -> {
                    // 安装期仅更新候选，等待 commit 统一发布。
                }

                State.READY -> {
                    if (candidate.isEmpty()) {
                        mutatePhysicalHook { group.physicalHandle?.unhook() }
                        groups.remove(group.key)
                    } else {
                        group.physicalHandle = installPhysicalHook(group, candidate, mutation = true)
                    }
                }
            ...
            group.logicalHooks.clear()
            group.logicalHooks.addAll(candidate)
        }

        private fun <T> mutatePhysicalHook(action: () -> T): T {
            EzXposed.markHotReloadIrreversible()
            return try {
                action()
                ...
                state = State.FAILED
                throw IllegalStateException(
                    "Physical Hook update failed; its outcome is unknown. Please restart the target process.",
                    failure,
                )
            }
        }

        private fun installPhysicalHook(
            group: HookGroup,
            logicalHooks: List<LogicalHook> = group.logicalHooks,
            mutation: Boolean = false,
        ): XposedInterface.HookHandle {
            val hooker = CompositeHooker(logicalHooks.map { it.hooker })
            val builder =
                xposed
                    .hook(group.key.executable)
                    .setPriority(group.key.priority)
                    .setExceptionMode(group.key.exceptionMode)
            // 构建失败尚未触及底层 hook；只有真正发布阶段的未知失败才使 batch 失效。
            val identified = XposedApiCompat.Api102.setId(builder, group.groupId)
            return if (mutation) mutatePhysicalHook { identified.intercept(hooker) } else XposedApiCompat.intercept(identified, hooker)
        }

        private fun groupIdFor(key: GroupKey): String =
            groupIdPrefix +
                sha256(
                    buildString {
        ...
        private fun describeTarget(target: Executable): String = "${target.declaringClass.name}#${target.name}"
        ...
        private data class HookIdentity(
            ...
            companion object {
                fun from(
                    ...
                    hookId: String?,
                ): HookIdentity? = hookId?.takeIf(String::isNotBlank)?.let { HookIdentity(executable, it) }
            }
        }
